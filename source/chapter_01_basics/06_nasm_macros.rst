...
1. 安装 NASM 并编写一个最小程序，仅调用 ``sys_exit`` 退出并返回 42。
   用 ``echo $?`` 验证退出码。

2. 在 ``hello.asm`` 的基础上，将字符串定义和 ``sys_write`` 调用封装成一个 ``%macro`` 宏。

3. 使用 ``%rep`` 生成一个包含 0, 1, 4, 9, 16, ..., 81 的平方数表格（共 10 项）。

4. 用 ``%ifdef`` 实现条件编译：当定义 ``DEBUG`` 宏时打印调试信息，
   否则只执行核心逻辑。

5. 将 ``SYS_READ``、``SYS_WRITE``、``SYS_EXIT`` 等系统调用号抽取到单独的
   ``syscalls.inc`` 文件，并用 ``%include`` 引入。
