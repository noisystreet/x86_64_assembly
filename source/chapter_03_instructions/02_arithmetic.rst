...
.. admonition:: 为什么 ``imul`` 有 3 种形式，而 ``mul`` 只有 1 种？
   :class: story

   8086 时代的 ``imul`` 只有单操作数形式（和 ``mul`` 一样隐含 ``rax``），
   因为当时晶体管数量极为有限。到了 386，Intel 引入了双操作数 ``imul``
   （``imul dst, src``），这是对编译器友好的重大改进——不再需要担心 ``rax`` 被占用。
   Pentium 时代又加入了三操作数形式（``imul dst, src, imm``），
   使得 ``imul rcx, rdx, 5`` 一条指令完成赋值 + 乘法。三种形式的并存是 **x86 向后兼容**
   理念的典型体现：从不删除旧功能，只在上面叠加新功能。
...
在进行有符号除法之前，必须将 ``rax``（被除数）符号扩展到 ``rdx:rax``。原因在于除法指令 ``div``/``idiv`` 使用的被除数始终是 128 位的 ``rdx:rax``——即使用不到，也必须正确设置高位。符号扩展指令就是专门做这件事的：
