...
1. 计算 ``0.1 + 0.2``，用 ``comiss`` 与 ``0.3`` 比较，验证浮点精度问题。
   用 ``subss`` 计算差值并取绝对值，判断是否小于 ``1.0e-6``。

2. 声明一个包含 4 个 ``float`` 的数组，用 ``addps`` 一次性完成向量加法。

3. 将 MXCSR 的舍入模式改为"向零舍入"后计算 ``1.0 / 3.0``，
   与默认舍入模式的结果进行比较。

4. 编写程序检测 ``0.0 / 0.0`` 产生 NaN，并用 ``ucomiss`` 的 PF 标志位检测。

5. 对比双精度和单精度的累加精度误差：重复 100 万次 ``x = x + 0.1``，
   分别用 ``float`` 和 ``double`` 实现，观察最终误差差异。
