...
1. 编写程序依次 ``push`` 五个不同的 64 位值，然后依次 ``pop`` 并观察出栈顺序。

2. 定义一个结构体，函数入口用 ``push rbp; mov rbp, rsp`` 建立栈帧，
   在栈上分配 32 字节局部空间，分别用 ``[rbp-8]`` 和 ``[rbp-16]`` 存取两个局部变量。

3. 用 ``sub rsp, N`` 代替 ``push`` 分配栈空间，并用 ``[rsp+offset]`` 方式访问。
   与使用 ``push`` 的方案进行比较，讨论各自的优缺点。

4. 编写一个递归函数，故意不设终止条件使其栈溢出，观察 ``SIGSEGV`` 的行为。
   然后修正函数使之正常工作。

5. 通过 ``ulimit -s`` 修改栈大小限制后运行相同程序，验证栈溢出阈值的变化。
