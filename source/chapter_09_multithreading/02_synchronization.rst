...
如果你从来没有写过并发程序，思考一个问题：**如果两个线程同时执行 ``counter++``（即 ``mov rax, [counter]; inc rax; mov [counter], rax``），会发生什么？** 可能的结果是：本来应该递增 2 次，但实际只递增了 1 次。原因是两个线程同时读取了相同的旧值，各自加 1 后写回，后写回的覆盖了先写回的。这就是 **数据竞争（data race）**。

同步原语的作用就是阻止这样的竞争。最基础的工具是 **互斥锁（mutex）**——它确保同一时间只有一个线程能进入"临界区"（操作共享数据的代码区域）。可以用一个简单的自旋锁开始理解其工作原理。自旋锁的核心是一个 **原子交换指令（``xchg``）**：检查锁是否空闲（值为 0），如果是则设置为 1 并进入；如果已被占用（值为 1），就不断旋转等待（"自旋"）。
